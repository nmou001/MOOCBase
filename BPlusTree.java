...
public class BPlusTree {
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        //Exception cases here:
        if (scanAll().hasNext()) {
            throw new BPlusTreeException("Tree is not empty!");
        }

        Optional<Pair<DataBox, Long>> optUpdatePair = root.bulkLoad(data, fillFactor);
        while (optUpdatePair.isPresent()) {
            Pair<DataBox, Long> updatePair = optUpdatePair.get();
            DataBox newRootKey = updatePair.getFirst();
            Long rightRootNum = updatePair.getSecond();
            ...
            InnerNode newRoot = new InnerNode(metadata, bufferManager, newRootKeys, newRootNums, lockContext);
            updateRoot(newRoot);
            optUpdatePair = root.bulkLoad(data, fillFactor);
        }
    }
