...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        int minStartCost = Integer.MAX_VALUE;
        for (String tableName: allTableNames) {
            Set<String> set = new HashSet<>();
            set.add(tableName);
            QueryOperator readOp = minCostSingleAccess(tableName);
            pass1map.put(set, readOp);

            // Keep track of which table name to start off with
            if (readOp.cost < minStartCost) {
                startTableName = tableName;
                minStartCost = readOp.cost;
            }
        }
        ...
        // been joined.
        Map<Set<String>, QueryOperator> map = pass1map;
        while (map.size() > 1) {
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int operator = minOp.getIOCost();

        for (Integer each : getEligibleIndexColumns(table)) {
            DataBox data = selectDataBoxes.get(each);
            QueryOperator scanindex = new IndexScanOperator(transaction, table,
                    selectColumnNames.get(each), selectOperators.get(each), data);
            if (scanindex.estimateIOCost() < minOp.estimateIOCost()) {
                operator = each;
                minOp = scanindex;

            }
        }
        return addEligibleSelections(minOp, operator);
    }
    ...
    Map<Set<String>, QueryOperator> minCostJoins(Map<Set<String>, QueryOperator> prevMap,
        ...
        for (Map.Entry<Set<String>, QueryOperator> key: prevMap.entrySet()) {
            Set<String> joinedTables = key.getKey();
            QueryOperator queryOp = key.getValue();
//            System.out.println("\tSeeking to find a join for set: ");
//            System.out.print("\t\t");
//            System.out.println(joinedTables);

            //FOR EACH join condition listed in the query
            for (int i = 0; i < joinRightColumnNames.size(); i++) {
                String[] leftNames = getJoinLeftColumnNameByIndex(i);
                String[] rightNames = getJoinRightColumnNameByIndex(i);
                boolean joinedLeftTable = joinedTables.contains(leftNames[0]);
                boolean joinedRightTable = joinedTables.contains(rightNames[0]);
                ...
                QueryOperator toAdd = null;
                String addedTable = "";
                if (joinedLeftTable && !joinedRightTable) {
                    searchSet.add(rightNames[0]);
//                    System.out.println("Joining with new table:" + searchSet);
                    toAdd = minCostJoinType(queryOp,
                            pass1Map.get(searchSet),
                            leftNames[1],
                            rightNames[1]);
                    addedTable = rightNames[0];
                } else if (!joinedLeftTable && joinedRightTable) {
                    searchSet.add(leftNames[0]);
//                    System.out.println("Joining with new table:" + searchSet);
                    toAdd = minCostJoinType(pass1Map.get(searchSet),
                            queryOp,
                            leftNames[1],
                            rightNames[1]);
                    addedTable = leftNames[0];
                }
                // If we satisfied a join condition, then add it
                if (toAdd != null) {
                    Set<String> plannedJoin = new HashSet<>();
                    plannedJoin.addAll(joinedTables);
                    plannedJoin.add(addedTable);
                    // We only to replace an earlier plan if we have a lower cost
                    if (map.containsKey(plannedJoin)) {
                        int currCost = map.get(plannedJoin).estimateIOCost();
                        if (toAdd.estimateIOCost() < currCost) {
                            map.put(plannedJoin, toAdd);
                        }
                    } else {
                        map.put(plannedJoin, toAdd);
                    }
