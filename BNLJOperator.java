        ...
        private void fetchNextRecord() {
            ...
            do {
                if (rightRecordIterator.hasNext()) {
                    DataBox leftJoin = leftRecord.getValues().get(getLeftColumnIndex());
                    Record rightRecord = rightRecordIterator.next();
                    DataBox rightJoin = rightRecord.getValues().get(getRightColumnIndex());
                    if (rightJoin.equals(leftJoin)) {
                        List<DataBox> leftValues = new ArrayList<>(leftRecord.getValues());
                        List<DataBox> rightValues = new ArrayList<>(rightRecord.getValues());
                        int counter = 0;
                        while (counter < rightValues.size()) {
                            leftValues.add(rightValues.get(counter));
                            counter++;
                        }
                        //leftValues.addAll(rightValues); why didn't it work
