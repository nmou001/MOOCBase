...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        // Not going to overflow, make sure no duplicate key additions
        int idxPut = InnerNode.numLessThanEqual(key, keys);
        ...
        if (keys.size() > 2*order) {
            ...
            assert (rightKeys.size() == order + 1);
            assert (rightRids.size() == order + 1);
            assert (keys.size() == order);
            assert (rids.size() == order);
            ...
            sync();
            Pair splitPair = new Pair(rightKeys.get(0), newRightLeaf.page.getPageNum());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int order = metadata.getOrder();
        int fillCapacity = (int)Math.ceil(2 * order * fillFactor);
        // Fill up to fill capacity, then split
        ...
        sync();
        // Split if we can add 1 more entry
        if (data.hasNext()) {
            Pair<DataBox, RecordId> pair = data.next();
            List<DataBox> rightKeys = Collections.singletonList(pair.getFirst());
            List<RecordId> rightRids = Collections.singletonList(pair.getSecond());
            LeafNode rSib = new LeafNode(metadata, bufferManager, rightKeys, rightRids, Optional.empty(), treeContext);
            rightSibling = Optional.of(rSib.getPage().getPageNum());
            return Optional.of(new Pair(rightKeys.get(0), rSib.page.getPageNum()));
        }
        return Optional.empty();
    }
    ...
    /**
     * Nethen: Received conceptual help from roommate to get started on this project.
     * Instead of using Optional to switch between values, he advised that I could just
     * check rightSibling's value.
     */
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> option = Optional.empty();
        ...
        //is Optional necessary here, need testing
        int sibling = -1;
        if (rightSibling != sibling) {
            option = Optional.of(rightSibling);
        }
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, option, treeContext);
