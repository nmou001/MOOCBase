...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        while (buildRecords.hasNext()) {
            ...
            DataBox key = buildRecord.getValues().get(buildColumnIndex);
        ...
        while (probeRecords.hasNext()) {
            ...
            if (hashTable.containsKey(probeKey)) {
                for (Record buildRecord : hashTable.get(probeKey)) {
                    Record joinedRecord = joinRecords(probeRecord, buildRecord, probeFirst);
    ...
    private List<Record> run(Iterator<Record> leftRecords, Iterator<Record> rightRecords, int pass) {
        ...
        List<Record> partitionJoined = new ArrayList<>();
        for (HashPartition partition: partitions) {
            ...
            if (partition.getNumLeftPages() < partitions.length) {
                partitionJoined = buildAndProbe(partition);
            } else if (partition.getNumRightPages() < partitions.length) {
                partitionJoined = buildAndProbe(partition);
            } else {
                //
                ...
                // to joinedRecords.
                //
                partitionJoined = run(partition.getLeftIterator(), partition.getRightIterator(), pass + 1);
            }

            joinedRecords.addAll(partitionJoined);
