...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        private LockContext getRoot(LockContext lockContext) {
        ...
        private LockContext getNextRelease(List<Lock> locks) {
            Lock toPop = null;
            LockContext toReturn = null;
            if (locks.size() == 0)
                return null;
            for (Lock lock: locks) {
        ...
        public void close() {
            ...
            // TODO(proj4_part2)
                ...
                if (locks != null) {
                    while (locks.size() > 0) {
                        LockContext toRelease = getNextRelease(locks);
                        toRelease.release(this);
                    }
                }
                //I suspect that we may have to release in reverse (according to spec) but I haven't tested it out yet, of course
    ...
    private class TransactionImpl extends AbstractTransaction {
        ...
        protected void startCommit() {
            ...
            //Haven't replaced this b/c previous tasks aren't completed yet
            //this.cleanup();
