...
class InnerNode extends BPlusNode {
    ...
    // See BPlusNode.put.
    /*
     *     I think we have to create two separate children for this
     *               |
     *               v
     *     +----+----+----+
     *     | 10 | 20 | 30 |
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2*order) {
            return Optional.of(splitMe(order));
        }
        sync();
        return Optional.empty();
    }

    // Splits node, returns right's pagenum, Calls sync()
    private Pair<DataBox, Long> splitMe(int order) {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (keys.size() == 0) {
            throw new BPlusTreeException("InnerNode is empty, something went wrong");
        }

        int order = metadata.getOrder();
        BPlusNode child = getChild(children.size() - 1);
        Optional<Pair<DataBox, Long>> optNextPair = child.bulkLoad(data, fillFactor);

        while (optNextPair.isPresent() && keys.size() < 2*order + 1) {
            Pair<DataBox, Long> nextPair = optNextPair.get();
            keys.add(nextPair.getFirst());
            children.add(nextPair.getSecond());

            child = getChild(children.size() - 1);
            optNextPair = child.bulkLoad(data, fillFactor);
        }
        sync();
